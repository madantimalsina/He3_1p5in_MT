...
{ 
  ...
  //MT-185 alloys of Tungsten used to make YBe source
  A = 183.84*g/mole;
  ...
  YBeTungsten_MT185->AddElement(elW,0.97);
  YBeTungsten_MT185->AddElement(elFe,0.009);
  YBeTungsten_MT185->AddElement(elNi,0.021);
...
G4VPhysicalVolume* He3DetectorConstruction::DefineVolumes()
{
  ...
  G4double hz_YBesHold = (24./2.)*cm;

/*  // Parameters: source holder plexiglass
  ...
  G4double gapThickness = 1.5*25.40*mm;  // Thickness of moderator (Varies: 1", 2", 2.5", 5" etc)
...
/*  //
  ...
  auto SBoxLV
    = new G4LogicalVolume(
                 SBox,             // its solid
                 //holdMaterial,
                 holdMaterial_Tungsten // For YBe Tungsten pig
                 //defaultMaterial,     // its material
                 "Source");           // its name
  ...
  //               
  G4Tubs* sourceTube = 
    new G4Tubs("Source", innerRadius_sHold, outerRadius_sHold, hz_YBesHold, startAngle, spanningAngle);

  G4LogicalVolume* logicRing =                         
    new G4LogicalVolume(sourceTube,           //its solid
                        holdMaterial_Tungsten,
                        //holdMaterial_Tungsten_MT185, 
                        //defaultMaterial,        //its material
                        "Source"); 
  ...
  fHoldPV
   = new G4PVPlacement(
                 0,                // no rotation
                 G4ThreeVector(0., 0., -(31.75*mm + hz_YBesHold)), // its position (gapThickness/2 + hz_YBesHold)
  ...
  fGapPV2
    = new G4PVPlacement(
                 0,                // no rotation
                 G4ThreeVector(0., 0., (58.7375 + (12.7 + 4.76 + gapThicknessB/2))*mm),
...
// below the tube (make a single slab for now)   
  ...
  fGapPV2
    = new G4PVPlacement(
                 0,                // no rotation
                 G4ThreeVector(0., 0., (58.7375 + (12.7 + gapThicknessB2/2))*mm),
                 //G4ThreeVector(0., 0., (gapThickness/2 + 2*outerRadius_THe3 + gapPEHe3T - 0.47625/2*cm + gapThicknessB/4.)), // 0.47625*cm -> wall thick of U -shape
...
//inner side wall of the U-channel (double thick wall)
  ...
  fGapPV2
    = new G4PVPlacement(
                 0,                // no rotation
                 G4ThreeVector(0., 31.7505*mm, gapThickness/2. + gapThicknessB2z/2.),
                 //G4ThreeVector(0., 0., (gapThickness/2 + 2*outerRadius_THe3 + gapPEHe3T - 0.47625/2*cm + gapThicknessB/4.)), // 0.47625*cm -> wall thick of U -shape
 ...
 // make a copy for other sides
  ...
  fGapPV2
    = new G4PVPlacement(
                 0,                // no rotation
                 G4ThreeVector(0., (31.7505 + 63.5)*mm, gapThickness/2. + gapThicknessB2z/2.), 
                 //G4ThreeVector(0., 0., (gapThickness/2 + 2*outerRadius_THe3 + gapPEHe3T - 0.47625/2*cm + gapThicknessB/4.)), // 0.47625*cm -> wall thick of U -shape
                 gapLV2s,            // its logical volume                         
                 "Gap2s",            // its name
                 layerLV,          // its mother  volume
    ...
    = new G4PVPlacement(
                 0,                // no rotation
                 G4ThreeVector(0., -31.75*mm, gapThickness/2. + gapThicknessB2z/2.), 
                 //G4ThreeVector(0., 0., (gapThickness/2 + 2*outerRadius_THe3 + gapPEHe3T - 0.47625/2*cm + gapThicknessB/4.)), // 0.47625*cm -> wall thick of U -shape
                 gapLV2s,            // its logical volume                         
                 "Gap2s",            // its name
                 layerLV,          // its mother  volume
    ...
    = new G4PVPlacement(
                 0,                // no rotation
                 G4ThreeVector(0., -(31.75 + 63.5)*mm, gapThickness/2. + gapThicknessB2z/2.), 
                 //G4ThreeVector(0., 0., (gapThickness/2 + 2*outerRadius_THe3 + gapPEHe3T - 0.47625/2*cm + gapThicknessB/4.)), // 0.47625*cm -> wall thick of U -shape
                 gapLV2s,            // its logical volume                         
                 "Gap2s",            // its name
                 layerLV,          // its mother  volume
  ...
  fGapPV2
    = new G4PVPlacement(
                 0,                // no rotation
                 G4ThreeVector(0., 152.4*mm, gapThickness/2. + gapThicknessB2z/2.),
                 //G4ThreeVector(0., 0., (gapThickness/2 + 2*outerRadius_THe3 + gapPEHe3T - 0.47625/2*cm + gapThicknessB/4.)), // 0.47625*cm -> wall thick of U -shape
...
// copy for the other side
  ...
  fGapPV
    = new G4PVPlacement(
                 rotD3,                // no rotation
                 G4ThreeVector((0.15 * 2.54)*cm, -112.71*mm, 58.7375*mm),
  ...
  fGapPV
    = new G4PVPlacement(
                 rotD3,                // no rotation
                 G4ThreeVector((0.15 * 2.54)*cm, 134.94*mm, 58.7375*mm), // its position
  ...
  fAbsorberPV
    = new G4PVPlacement(
                 rotD3,                // no rotation
                 G4ThreeVector((0.15 * 2.54)*cm, (0.5625*2.54)*cm, 58.7375*mm), // its position Centre tube is off by 0.635cm (0.25")
  ...
  fAbsorberPV1
    = new G4PVPlacement(
                 rotD3,                // no rotation
                 G4ThreeVector((0.15 * 2.54)*cm, -(112.71)*mm, 58.7375*mm), // its position
  ...
  fHoldPV
    = new G4PVPlacement(
                 0,                // no rotation
                 G4ThreeVector(0., calorSizeY/2. + 2.7*cm, -(layerThickness - gapThickness -(3.25*2.54)*cm + 42.*mm)), // its position ( - gapThicknessSW)
  ...
  fHoldPV
    = new G4PVPlacement(
                 rotD1,                // no rotation
                 G4ThreeVector(calorSizeY/2. + ((0.5+1.25)*2.54)*cm, 0., -(layerThickness - gapThickness - (3.25*2.54)*cm + 42.*mm)), // its position ()gapThicknessB
